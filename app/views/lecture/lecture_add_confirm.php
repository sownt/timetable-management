    ...
    <div class="container p-5">
        <form method="POST">
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="lectureName" value="<?= $_SESSION['name'] ?>" disabled>
                </div>
            </div>
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <select class="form-select" aria-label="Default select example" disabled>
                        <option selected><?php echo "Năm " . $_SESSION['school_year']; ?></option>
                    </select>
                </div>
            </div>
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" disabled><?= $_SESSION['description'] ?></textarea>
                </div>
            </div>
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    ...
                    <?php } else { ?>
                        <img src="<?php echo $_SESSION['avatar']; ?>" height="200">
                    <?php } ?>
                </div>
            </div>
            <?php
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                Subject::add($name, $target_file, $description, $school_year);
                header("Location: ./?controller=lecture&action=register_complete");
            }
            ?>
        ...
        <div class="text-center mt-3">
            <button class="btn btn-secondary" id="edit" onclick="return false;">Sửa lại</button>
        </div>
        <script type="text/javascript">
            document.getElementById("edit").onclick = function() {
                window.location.href = location.protocol + '//' + location.host + location.pathname + "?controller=lecture&action=register";
            };
        </script>
    </div>
